...
public class RingView extends View {
    ...
    private static int startColor = 0XFFfb5338;
    private static int centerColor = 0XFF00ff00;
    private static int endColor = 0XFF008dfc;
    private static int endUseColor = 0;
...
//            pointPaint.setShadowLayer(pointShadowLayer, 0, 0, Color.WHITE);//设置阴影
     ...
     * 采矿量
     */
    public static void startCkl(int value, RingView ringView) {
        ...
        ringView.setValue(value, new RingView.OnProgerssChange() {
            @Override
...
//                ly_content.setBackgroundColor(evaluate);
     ...
     * 剥离量
     */
    public static void startBll(int value, RingView ringView) {
        float f = (value - valueList.get(0)) * 1.0f / (valueList.get(valueList.size() - 1) - valueList.get(0));
        if (f <= 0.5f) {
            endUseColor = (Integer) evaluator.evaluate(f, startColor, centerColor);
        } else {
